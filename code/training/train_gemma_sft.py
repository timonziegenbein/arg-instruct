...
if device_string == "0":
    ...
    wandb.login()
    run = wandb.init(
        project='Fine-tune Gemma-2-9b on seed tasks',
        job_type="training",
        anonymous="allow"
    )
...
dataset_name_val = "timonziegenbein/ca_all_merged52k"
dataset_val = load_dataset(dataset_name_val, split="validation")
...
def _prepare_non_packed_dataloader(
    ...
    def tokenize(element):
        ...
        if validation:
            element[dataset_text_field] = [x.split("<start_of_turn>model\n")[0] + "<start_of_turn>model\n" if formatting_func is None else formatting_func(x.split("<start_of_turn>model\n")[0]) + "<start_of_turn>model\n"  for x in element[dataset_text_field]]
            ...
            return {"input_ids": outputs["input_ids"], "labels": outputs["labels"], "attention_mask": outputs["attention_mask"], "source": element["source"]}
        else:
    ...
    tokenized_dataset.filter(lambda x: ','.join([str(x) for x in [106, 2516, 108]]) in ','.join([str(x) for x in x["input_ids"]]), num_proc=4)

    sources = [] 
    if validation:
        sources = tokenized_dataset["source"]
        tokenized_dataset = tokenized_dataset.remove_columns(["source"])

    print(len(tokenized_dataset))
    print(len(sources))

    return tokenized_dataset, sources


dataset_train, _ = _prepare_non_packed_dataloader(
    tokenizer,
    dataset_train,
    "text",
    512,
...
dataset_val, val_sources = _prepare_non_packed_dataloader(
    tokenizer,
    dataset_val,
    "text",
    512,
...
def compute_metrics(eval_preds, sources):
    #dataset_val = load_dataset(dataset_name_val, split="validation")
    #dataset_val = dataset_val.select(range(2))
    #print(sources)
    #print(eval_preds)
    ...
    sources = [sources[i] for i in range(len(sources)) if decoded_preds_split[i] is not None]
    ...
    for source in set(sources):
        ...
        for i in range(len(decoded_preds_source)):
            rougeL_source += default_rouge_scorer.score(decoded_labels_source[i], decoded_preds_source[i])['rougeL'].fmeasure
...
training_arguments = Seq2SeqTrainingArguments(
    output_dir='/mnt/home/mstahl/argpaca/models/gemma-2-9b-it-ca52k',
...
trainer = Seq2SeqTrainerWithTwoDataCollators(
    ...
    #packing=False,
    compute_metrics=partial(compute_metrics, sources=val_sources),
