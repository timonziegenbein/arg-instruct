...
if __name__ == "__main__":
    ...
    for task in set(data['task']):
        for metric in ['micro_f1', 'macro_f1', 'mase', 'rougeL', 'exact_matches']:
    ...
    df.to_csv('evaluation_results_balanced.csv', index=False)

    # calculate average values for each approach 
    pivot_df = df.pivot_table(index='approach', values=['micro_f1', 'micro_f1_rank', 'mase', 'mase_rank', 'rougeL', 'rougeL_rank'], aggfunc='mean', columns='split')
    # undo the multiindex
    pivot_df.columns = ['_'.join(col).strip() for col in pivot_df.columns.values]
    pivot_df = pivot_df[['micro_f1_test', 'micro_f1_rank_test', 'mase_test', 'mase_rank_test', 'rougeL_test', 'rougeL_rank_test', 'micro_f1_train', 'micro_f1_rank_train', 'mase_train', 'mase_rank_train', 'rougeL_train', 'rougeL_rank_train']]
    # calc mean of ranks
    pivot_df['mean_rank_test'] = pivot_df[['micro_f1_rank_test', 'mase_rank_test', 'rougeL_rank_test']].mean(axis=1)
    pivot_df['mean_rank_train'] = pivot_df[['micro_f1_rank_train', 'mase_rank_train', 'rougeL_rank_train']].mean(axis=1)
    # round to 2 decimal places
    pivot_df.to_csv('evaluation_results_balanced_pivot_task.csv')
    ...
    for task in set(dataset_df['dataset']):
        for metric in ['micro_f1_rank', 'mase_rank', 'rougeL_rank']:
    ...
    pivot_df = dataset_df.pivot_table(index='approach', values=['micro_f1', 'micro_f1_rank', 'mase', 'mase_rank', 'rougeL', 'rougeL_rank'], aggfunc='mean', columns='split')
    # undo the multiindex
    pivot_df.columns = ['_'.join(col).strip() for col in pivot_df.columns.values]
    pivot_df = pivot_df[['micro_f1_test', 'micro_f1_rank_test', 'mase_test', 'mase_rank_test', 'rougeL_test', 'rougeL_rank_test', 'micro_f1_train', 'micro_f1_rank_train', 'mase_train', 'mase_rank_train', 'rougeL_train', 'rougeL_rank_train']]
    # create rank of ranks
    pivot_df['micro_f1_rank_test'] = rankdata(pivot_df['micro_f1_rank_test'], method='dense')
    pivot_df['mase_rank_test'] = rankdata(pivot_df['mase_rank_test'], method='dense')
    pivot_df['rougeL_rank_test'] = rankdata(pivot_df['rougeL_rank_test'], method='dense')
    pivot_df['micro_f1_rank_train'] = rankdata(pivot_df['micro_f1_rank_train'], method='dense')
    pivot_df['mase_rank_train'] = rankdata(pivot_df['mase_rank_train'], method='dense')
    ...
    pivot_df.to_csv('evaluation_results_balanced_pivot_dataset.csv')
